function dydt = ODE_metabolator_mod(Time, y0, k1, k2, k3, Km1, Km2, S0, kTCA, kAckf, kAckr, C, Hp, Keq, HOAcE, a0, a1, a2, a3, Kg1, Kg2, Kg3, kd, n, beta_max, B, B_half)

    beta = 1 - (beta_max - 1)./(1 + exp(B_half)) + (beta_max - 1)./(1 + exp(-(B-B_half)));
    ...
    dydt = [((k2.*y0(7).*y0(3))./(Km2 + y0(3)) - (k1.*y0(6).*y0(1))./(Km1 + y0(1)) + S0 - kTCA.*y0(1));
            ...
            ((a1.*(y0(2)./(beta*Kg1)).^n)./(1+(y0(2)./(beta*Kg1)).^n)+a0 - kd.*y0(5));
